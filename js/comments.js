...
function sendComment(data) {
  ...
  request.open(
    "POST",
    "https://neto-api.herokuapp.com/pic/" + imageID + "/comments",
    true
  );
...
function renderComment(comment) {
  const currentFormNode = document.querySelector(
    `.comments__form[data-left="${comment.left}"][data-top="${comment.top}"]`
  );
  if (currentFormNode) {
...
function renderNewCommentElement(currentFormNode, comment) {
  const currentFormNodeCommentsBody = currentFormNode.querySelector(
    ".comments__body"
  );
  const currentFormNodeLoader = currentFormNode.querySelector(
    ".comment_loader"
  );
  const commentsFormSimple = currentFormNodeCommentsBody.querySelector(
    ".comment"
  );
  const commentElement = commentsFormSimple.cloneNode(true);
  let date = new Date();
  ...
  commentMessage.setAttribute("style", "white-space: pre;");
  commentMessage.textContent = comment.message;
  currentFormNodeCommentsBody.insertBefore(
    commentElement,
    currentFormNodeLoader
  );
